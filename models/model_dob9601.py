...
class CIFARModel(BaseModel):
    ...
    def __init__(self, dataset: Dataset, testset: Dataset):
        ...
        self.conv = nn.Sequential(
            nn.BatchNorm2d(3),
            nn.Conv2d(3, 64, 3, padding=2),  # 32px
            nn.ReLU(),
            nn.MaxPool2d(4, 2),   # Halves image size    (16px)
            nn.Conv2d(64, 128, 3, padding=2),  # 16px
            nn.ReLU(),
            nn.MaxPool2d(3, 2),   # 8px
        )
    ...
    def forward(self, x: Tensor) -> Tensor:
        x = self.conv(x)
        x = x.view(-1, 128 * 8 * 8)  # -1 means infer this dimension
        x = self.linear(x)
        return x
